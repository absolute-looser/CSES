...
ll ans = INT_MAX;
ll t1,t2;
void check(ll a[],int l,int r,ll sum,ll total){
    if(l==r)
        return;
    t1 = total-(sum+a[l]);
    t2 = total-sum;
    ans = min(min(abs(sum+a[l]-t1),abs(sum-t2)),ans);
    //cout << ans << endl;
    check(a,l+1,r,sum+a[l],total);
    check(a,l+1,r,sum,total);
}
void run(){    
    int n;
    cin >> n;
